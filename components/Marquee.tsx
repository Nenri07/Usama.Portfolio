...
import { wordmarks } from '@/lib/data';
import { buildMarqueeTrack } from '@/lib/format';

/**
 * Marquee — the "Trusted By" section (Req 3).
 *
 * - Renders the 7 wordmarks as styled TEXT (no images/logos) in EXACT order,
 *   separated by a fixed inter-wordmark gap and a maroon dot separator
 *   (Req 3.1, 3.2, 3.3).
 * - Seamless infinite loop via a DUPLICATED track: `buildMarqueeTrack` returns
 *   `[...wordmarks, ...wordmarks]`. anime.js animates the track `translateX`
 *   from 0 to -50% with `loop: true`, `ease: 'linear'`, constant duration, so
 *   the scroll is continuous with no pause between iterations. Because the two
 *   halves are identical, the seam at -50% is invisible (Req 3.4, 3.5).
 * - Graceful default (Req 3.6): the track's default DOM state is translateX 0,
 *   which already shows all 7 wordmarks fully readable. `animate()` is wrapped
 *   in try/catch; if anime.js fails to init, the animation simply never starts
 *   and the wordmarks stay static and legible.
 */

// One rendered copy of the wordmark sequence, used for both halves of the track.
const HALF = wordmarks;
...
export default function Marquee() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // so all 7 wordmarks render static and fully readable (Req 3.6).
    }
  }, []);
    ...
    >
      ...
      <div className="w-full overflow-hidden">
        {/* Duplicated track — width is content-driven; translateX -50% moves it
            by exactly one half (one full wordmark sequence). */}
        <div
          ref={trackRef}
        ...
        >
          {TRACK.map((mark, i) => (
            ...
            >
              ...
              {/* Maroon dot separator — the only accent, kept sparing (Req 7.1). */}
              <span
                aria-hidden="true"
